...
public class Affichage {
  ...
  /**
   * afficherCasesDeplacementsPossible
   *
   * @param x
   * @param y
   */
  public static void afficherCasesDeplacementsPossible(int x, int y) {
    StdDraw.setPenColor(new Color(0, 255, 0, 70));
    StdDraw.filledRectangle(
      Config.largeurCaseX - 0.005 + Config.largeurCaseX * x,
      Config.largeurCaseY - 0.005 + Config.largeurCaseY * y,
      Config.largeurCaseX / 2,
      Config.largeurCaseY / 2
    );
  }

  /**
   * afficherCasesAttaquesPossible
   *
   * @param x
   * @param y
   */
  public static void afficherCasesAttaquesPossible(int x, int y) {
    StdDraw.setPenColor(new Color(255, 0, 0, 50));
    StdDraw.filledRectangle(
      Config.largeurCaseX - 0.005 + Config.largeurCaseX * x,
      Config.largeurCaseY - 0.005 + Config.largeurCaseY * y,
      Config.largeurCaseX / 2,
   ...
   * @param y
   */
  public static void afficherCasesAttaquesPossibleMnt(int x, int y) {
   ...
   */
  private static double positionInterneRectangleX(
    double gauche,
    double droite,
    double haut,
    double bas,
   ...
   */
  private static double positionInterneRectangleY(
    double gauche,
    double droite,
    double haut,
    double bas,
  ...
  private static double positionInterneCarteX(double x) {
    return (
      positionInterneRectangleX(
        Config.limiteGaucheCarte,
        Config.limiteDroiteCarte,
        Config.limiteHautCarte,
        Config.limiteBasCarte,
        x
      )
    );
  }
  ...
  public static double coordXCentreCase(int x) {
    return (
      positionInterneCarteX(
        ((double) x + 0.5) / (double) Config.longueurCarteXCases
      )
    );
  }
  ...
  public static double coordYCentreCase(int y) {
    return (
      positionInterneCarteY(
        ((double) y + 0.5) / (double) Config.longueurCarteYCases
      )
    );
  }
   ...
   */
  public static void dessineContourRectangle(
    double gauche,
    double droite,
    double haut,
    double bas,
   ...
   */
  public static void dessineRectanglePlein(
    double gauche,
    double droite,
    double haut,
    double bas,
   ...
   */
  public static void afficheTexteDansCase(
    int xCase,
    int yCase,
    String texte,
    Color couleur,
    ...
    StdDraw.setFont(f);
    StdDraw.text(
      coordXCentreCase(xCase) + (xInterne - 0.5) * Config.largeurCaseX,
      coordYCentreCase(yCase) + (yInterne - 0.5) * Config.largeurCaseY,
      texte
    );
  }
   ...
   */
  private static void dessineLigne(
    double departX,
    double departY,
    double arriveeX,
    double arriveeY,
    ...
    for (int vertical = 0; vertical <= 1; vertical++) {
      for (int gauche = 0; gauche <= 1; gauche++) {
        for (int haut = 0; haut <= 1; haut++) {
          ...
          double finX =
            debutX + longueurX * (horizontal * gauche - droite * horizontal);
  ...
  public static void videCase(int x, int y) {
    ...
    StdDraw.filledRectangle(
      coordXCentreCase(x),
      coordYCentreCase(y),
      0.95 * Config.demiLargeurCaseX,
      0.95 * Config.demiLargeurCaseY
    );
  }
 ...
 */
  public static void videZoneTexte() {
    dessineRectanglePlein(
      Config.limiteGaucheTexte,
      Config.limiteDroiteTexte,
      Config.limiteHautTexte,
      Config.limiteBasTexte,
      Config.COULEUR_ARRIERE_PLAN
    );
  }
 ...
 */
  public static void dessineGrille() {
    for (int i = 0; i <= Config.longueurCarteXCases; i++) {
      dessineLigne(
        positionInterneCarteX((double) i / (double) Config.longueurCarteXCases),
        positionInterneCarteY(0),
        positionInterneCarteX((double) i / (double) Config.longueurCarteXCases),
        positionInterneCarteY(1),
        Color.black
      );
    }
    for (int i = 0; i <= Config.longueurCarteYCases; i++) {
      dessineLigne(
        positionInterneCarteX(0),
        positionInterneCarteY(
          ((double) i) / (double) Config.longueurCarteYCases
        ),
        positionInterneCarteX(1),
        positionInterneCarteY(
          ((double) i) / (double) Config.longueurCarteYCases
        ),
        Color.black
  ...
  public static int popup(
    ...
    String[] options,
    boolean echapAutorise,
    int curseurDefaut
  ) {
    int nbOptions = options.length;
    double shareForOption =
      Config.PIXELS_PAR_ITEM_POPUP / (double) Config.nbPixelsY;
    ...
    if (height > Config.carteTailleY) {
      ...
      nombreMaxDOptionsAffichees =
        (int) Math.floor(height / shareForOption - 2.0);
    }
    ...
    double explainationY =
      bottom + (nombreMaxDOptionsAffichees + 1.0) * shareForOption; //ordonne du texte explicatif
    ...
    do {
      dessineRectanglePlein(
        ...
        top,
        bottom,
        Config.POPUP_COULEUR_ARRIERE_PLAN
      );
      dessineContourRectangle(
        left,
        right,
        top,
        bottom,
        Config.POPUP_COULEUR_CADRE
      );
      StdDraw.setFont(Config.POLICE_POPUP);
      StdDraw.setPenColor(Config.POPUP_COULEUR_TEXTE); // couleur du texte de popup
      StdDraw.textLeft(
        0.95 * left + 0.05 * right,
        explainationY,
        messageExplicatif
      );
      for (int i = 0; i < nombreMaxDOptionsAffichees; i++) StdDraw.textLeft(
        XOption,
        topPosOptions[i],
        options[i + offsetOptions]
      );
      StdDraw.show();
      ...
      pixelsCursor =
        Math.min(
          0.1 * (right - left) / Config.largeurCaseX,
          shareForOption / Config.largeurCaseY
        ); //pour s'assurer que l'image n'est pas deformee
      widthX = pixelsCursor * Config.largeurCaseX;
      widthY = pixelsCursor * Config.largeurCaseY;
      StdDraw.picture(
        centerX,
        centerY,
        Chemins.CHEMIN_CURSEUR_POPUP,
        widthX,
        widthY
      );
      StdDraw.show();
      ...
      if (key.isBas()) {
        ...
        if (
          popupCursor == nombreMaxDOptionsAffichees - 2 &&
